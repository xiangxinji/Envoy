...
- [x] 1.1 创建 `src/teamwork/` 目录
- [x] 1.2 在 `src/index.ts` 中添加 teamwork 模块导出

## 2. 核心类型定义

- [x] 2.1 创建 `src/teamwork/types.ts`，定义 ResourceChange、ResourceQuery、ResourceAck 等资源操作相关类型
- [x] 2.2 定义 TeamOptions、LeaderOptions、MemberOptions 配置接口

## 3. Team 类实现

- [x] 3.1 创建 `src/teamwork/team.ts`，实现 Team 类封装 Server，支持 resourceRoot 配置
...
- [x] 4.1 创建 `src/teamwork/leader.ts`，实现 Leader 类封装 Client
...
- [x] 5.1 创建 `src/teamwork/member.ts`，实现 Member 类封装 Client
...
- [x] 6.1 在 `src/teamwork/index.ts` 中导出 Team、Leader、Member 及所有类型，再由 `src/index.ts` 统一导出
- [x] 6.2 验证 TypeScript 编译通过
- [x] 6.3 创建基础示例文件 `examples/teamwork-basic.ts` 演示完整流程
